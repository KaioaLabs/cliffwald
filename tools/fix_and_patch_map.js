...
// Fix the corrupted injection: replace \" with "
content = content.replace(/\\\"/g, '"');

// Also remove the double "layers": [ if it happened
// It looks like: "layers": [ { ... }, { "data": ...
// That part seems okay as long as quotes are fixed.

try {
    const map = JSON.parse(content);
    console.log("JSON is now valid. Proceeding with FixedSeats injection...");
    ...
    // Replace the corrupted layer if needed or just unshift
    // The previous unshift might have failed, but let's check
    map.layers = map.layers.filter(l => l.name !== "FixedSeats");
    map.layers.unshift(newLayer);

    fs.writeFileSync(mapPath, JSON.stringify(map, null, 1));
    console.log("Successfully fixed and updated world.json");

} catch (e) {
    console.error("Critical error parsing cleaned JSON:", e);
    fs.writeFileSync('corrupted_map_debug.json', content);
}
