...
import { getDialoguePool } from "../data/DialogueRegistry";
...
export const AISystem = (
    ...
    for (const entity of entities) {
                    ...
                    physicsWorld.intersectionsWithShape(currentPos, 0, PREFECT_VISION, (collider) => {
                        ...
                        if (victimId && !victimId.startsWith('prefect_') && !victimId.startsWith('teacher_')) {
                            ...
                            if (dist < 30) {
                                ...
                                const ray = new RAPIER.Ray(currentPos, rayDir);
                                ...
                                const interactionGroups = 0x00010001 as any; 
        ...
        if (chatCallback && Math.random() < 0.0005) {
             const pool = getDialoguePool(ai.archetype || 'SOCIALIZER');
        ...
        if (distToTarget > 20) {
            ...
            if (targetChanged) {
             ...
             if (ai.state !== 'idle' && ai.state !== 'attending_class') {
                 ...
                 const schedule = ai.routineSpots ? getStudentScheduleTarget(numericId, currentHour, ai.routineSpots) : null;
            ...
            if (ai.path && ai.path.length > 0) {
                ...
                if (distToNext < 12) {
                    ...
                    if (ai.noiseTimer > 500) { // Update noise every 0.5s
                        let noiseScale = 0;
                        if (ai.archetype === 'SOCIALIZER') noiseScale = 0.4; // Casual weaving
                        if (ai.archetype === 'EXPLORER') noiseScale = 0.8; // Looking around / drunk walk
                        if (ai.archetype === 'KILLER') noiseScale = 0.2; // Twitchy correction
