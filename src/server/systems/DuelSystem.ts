        ...
        this.room.entities.forEach((entity, id) => {
            ...
            if (dist < zone.radius) {
                ...
                if (playerState.inDuel) {
                    ...
                    playerState.duelScore = 0;
                    if (entity.ai) entity.ai.state = 'idle';
                }
            }
        });
    }

    private assignDuelTarget(entity: any, myId: string) {
        ...
        this.room.state.players.forEach((p, id) => {
            if (id === myId || !p.inDuel) return;
            // House Rivalry: Only attack different houses
            if (p.house === myState.house) return;
